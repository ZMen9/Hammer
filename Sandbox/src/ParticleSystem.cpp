...
void ParticleSystem::OnUpdate(hammer::Timestep ts) {
...
void ParticleSystem::OnRender(hammer::OrthographicCamera& camera) {
  hammer::Renderer2D::BeginScene(camera);
  for (auto& particle : m_ParticlePool) {
    if (!particle.Active) continue;
    ...
    hammer::Renderer2D::DrawRotatedQuad(position, {size, size},
